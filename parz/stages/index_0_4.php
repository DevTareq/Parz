...
function getHeaders()
{
    ...
    $fp = fopen('_data/orders_0_4.iif', 'r');
...
function mergeHeadersValues($headers, $values)
{
    ...
    if (!empty($headers['recordHeaders'])) {

        foreach ($values['headersValues'] as $header) {

            $count = 0;
            foreach ($header as $innerHeader) {

                $tempHeaders[$headers['recordHeaders'][$count]] = $innerHeader;
                $count++;

            }

            $mergedValues['headersValues'][] = $tempHeaders;
        }
    }


    if (!empty($headers['recordNestedHeaders'])) {
